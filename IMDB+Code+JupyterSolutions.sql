...
from movie group by year order by year; 

Select month(date_published) as month_num , COUNT(title) as number_of_movies
FROM movie group by month_num order by COUNT(title) desc;
...
from movie where (country like '%USA%' or country like '%India%') and year = 2019 ;
...
select count(movie_id) as only_one_genre_movies_count
from one_genre;
...
from genre as g inner join movie as m on g.movie_id=m.id
group by genre order by avg_duration ;
...
select * from ranking where movie_rank<=10;
...
select  production_company , movie_count , prod_company_rank from ranking where prod_company_rank = 1 ;
...
from movie as m inner join genre as g on m.id=g.movie_id inner join ratings as r on g.movie_id = r.movie_id
...
select count(movie_id) as movie_count_with_rating_more_than_8
...
German_count as
(
select sum(total_votes) as GERMAN_movie_votes
from language 
where languages regexp 'German'
)
select GERMAN_movie_votes, sum(total_votes) as Italian_movie_votes
from language , German_count
where languages regexp 'Italian';

-- here we are including the votes which were received for both German and Italian movies i.e. votes in common for both languages are included.

-- so we can also use union to get the number of votes neglecting the common number of voted both the languages movie received as follows.
...
from movie as m inner join director_mapping as dm on m.id = dm.movie_id
...
group by name having count(m.id)>=5
limit 1;
...
select * from highest_five where movie_rank <=5;
...
with directors as
(
select name_id as director_id, name as director_name, dir.movie_id, avg_rating , total_votes ,
 avg_rating * total_votes as rating_count, duration ,date_published, 
lead(date_published, 1) over (partition by name order by date_published, name) as next_publish_date
from director_mapping as dir inner join names as n on dir.name_id = n.id
inner join movie as mov on dir.movie_id = mov.id 
inner join ratings as r on mov.id = r.movie_id
)
